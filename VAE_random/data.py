...
class test_dataset_gt5:
    def __init__(self, image_root, gt_root, gt_root1, gt_root2, gt_root3, gt_root4, testsize):
        self.testsize = testsize
        self.images = [image_root + f for f in os.listdir(image_root)  if f.endswith('.jpg')
                    or f.endswith('.png')]
        self.gts = [gt_root + f for f in os.listdir(gt_root) if f.endswith('.jpg')
    ...
    def load_data(self):
        ...
        gt1 = self.binary_loader(self.gts1[self.index])
        gt2 = self.binary_loader(self.gts2[self.index])
        gt3 = self.binary_loader(self.gts3[self.index])
        gt4 = self.binary_loader(self.gts4[self.index])
        ...
        if name.endswith('.bmp'):
            name = name.split('.bmp')[0] + '.png'
        self.index += 1
        return image, gt, gt1, gt2, gt3, gt4, HH, WW, name

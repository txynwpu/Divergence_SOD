...
class Pred_endecoder(nn.Module):
    ...
    def __init__(self, channel, latent_dim):
        ...
        self.dec_pred = Pred_decoder(channel, latent_dim)
    ...
    def forward(self, x, y1=None, y2=None, y3=None,y4=None, y5=None, training=True):
        raw_x = x
        ...
        mj_pred = self.dec_pred(x1,x2,x3,x4)

        if training:
            prior, mux, logvarx = self.x_encoder(raw_x)
            random_rot_index = torch.randint(1, 6, (1,))
            if random_rot_index == 1:
                gt_cur = y1
                posterior, muxy, logvarxy = self.xy_encoder(torch.cat((raw_x, y1), 1))
            elif random_rot_index == 2:
                gt_cur = y2
                posterior, muxy, logvarxy = self.xy_encoder(torch.cat((raw_x, y2), 1))
            elif random_rot_index == 3:
                gt_cur = y3
                posterior, muxy, logvarxy = self.xy_encoder(torch.cat((raw_x, y3), 1))
            elif random_rot_index == 4:
                gt_cur = y4
                posterior, muxy, logvarxy = self.xy_encoder(torch.cat((raw_x, y4), 1))
            else:
                gt_cur = y5
                posterior, muxy, logvarxy = self.xy_encoder(torch.cat((raw_x, y5), 1))

            latent_loss = torch.mean(self.kl_divergence(posterior, prior))
            z_prior = self.reparametrize(mux, logvarx)
            z_posterior = self.reparametrize(muxy, logvarxy)
            pred_posterior = self.dec_pred(x1, x2, x3, x4, z_posterior)
            pred_prior = self.dec_pred(x1, x2, x3, x4, z_prior)
            return mj_pred, pred_prior, pred_posterior, latent_loss, gt_cur

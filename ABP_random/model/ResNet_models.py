...
class Encode_x(nn.Module):
    def __init__(self, input_channels, channels, latent_size):
        ...
        self.fc1 = nn.Linear(channels * 8 * 11 * 11, latent_size)  # adjust according to input size
        self.fc2 = nn.Linear(channels * 8 * 11 * 11, latent_size)  # adjust according to input size
    ...
    def forward(self, input):
        ...
        output = output.view(-1, self.channel * 8 * 11 * 11)  # adjust according to input size
        # print(output.size())
...
class Pred_endecoder(nn.Module):
    ...
    def __init__(self, channel, latent_dim):
        ...
        self.noise_conv = nn.Conv2d(channel + latent_dim, channel, kernel_size=3, padding=1)
        self.spatial_axes = [2, 3]

        self.encx = Encode_x(3,channel,latent_dim)
    ...
    def forward(self, x, z=None):
        ...
        pred = self.dec_pred(x1,x2,x3,x4,z)
